...
async def _classify(message: str) -> str:
    """Classifies the given message using a safety model.

Args:
    message (str): The message to classify.

Returns:
    str: The classified content in uppercase or 'UNSAFE' if an error occurs.
    
Raises:
    SafetyClassificationError: If there is an HTTP status error from ShieldGemma.
    ...
    try:
        ...
        raise SafetyClassificationError(f'ShieldGemma returned HTTP {e.response.status_code}') from e
    except httpx.RequestError as e:
        raise SafetyClassificationError(f'ShieldGemma unreachable or malformed response: {e}') from e
    except (KeyError, IndexError, TypeError, AttributeError, ValueError):
        return 'UNSAFE'

async def is_safe(message: str) -> bool:
    """Determines if a message is safe based on its content.

Args:
    message (str): The message to be evaluated for safety.
